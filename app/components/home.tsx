...
export default function Home({ fetchedGroups }: { fetchedGroups: Group[] }) {
  // const [groups, setGroups] = useState<Group[]>(mockGroups);
  const [groups, setGroups] = useState<Group[]>(fetchedGroups);
  ...
  const handleOpenAll = (group: Group) => {
    ...
    // ポップアップブロッカーを回避するため、同期的に全てのウィンドウを開く
    const windows = group.links.map((link) => {
      return window.open(link.url, "_blank");
    });

    // 開けなかったウィンドウがあるかチェック
    const blockedCount = windows.filter((w) => w === null).length;
    ...
    if (blockedCount > 0) {
      toast("一部のリンクがブロックされました", {
        description: `${group.links.length}個中${
          group.links.length - blockedCount
        }個のリンクを開きました。ブラウザのポップアップ設定を確認してください。`,
      });
    } else {
      toast("リンクを開きました", {
  ...
  //       }
  //     } catch (error) {
  //       console.error("Failed to open link:", link.url, error);
  //     }
  //   }
  ...
  //   if (openedCount < group.links.length) {
  //     toast("一部のリンクがブロックされました", {
  //       description: `${group.links.length}個中${openedCount}個のリンクを開きました。ポップアップを許可してください。`,
  //       action: {
  //         label: "ヘルプ",
  //         onClick: () => {
  //           alert(
  //             "ブラウザのアドレスバー右側のアイコンをクリックして、このサイトのポップアップを「常に許可」に設定してください。"
  //           );
  //         },
  //       },
  //     });
  ...
  const handleDeleteGroup = async (groupId: string) => {
    const res = await fetch(`/api/firebase/group?groupId=${groupId}`, {
      method: "DELETE",
    });
    const result = await res.json();
    if (result.status === 200) {
  ...
  const handleDuplicateGroup = async (group: Group) => {
    ...
    if (result.status === 200) {
      ...
      toast("グループの複製に失敗しました");
      console.log("グループの複製に失敗", result.message);
    }
  };
